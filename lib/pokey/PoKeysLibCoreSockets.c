...
// Get a list of interfaces - source: Jeremy @
...
uint32 *GetBroadcastAddresses() {
  ...
  *ptr = 0;
  ...
  // http://msdn2.microsoft.com/en-us/library/aa365917.aspx Now get Windows'
  // IPv4 addresses table.  Once again, we gotta call GetIpAddrTable() multiple
  // times in order to deal with potential race conditions properly.
  {
    for (i = 0; i < 5; i++) {
...
int32_t PK_EnumerateNetworkDevices(sPoKeysNetworkDeviceSummary *devices,
                                   uint32_t timeout) {
  return PK_SearchNetworkDevices(devices, timeout, 0);
}

int32_t PK_SearchNetworkDevices(sPoKeysNetworkDeviceSummary *devices,
                                uint32_t timeout, uint32_t serialNumberToFind) {
  ...
  if (setsockopt(txSocket, SOL_SOCKET, SO_BROADCAST, (char *)&UDPbroadcast,
                 sizeof UDPbroadcast) == -1) {
    debug_printf("Error setting broadcast option\n");
    return 0;
  }

#ifdef WIN32
  if (setsockopt(txSocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&t, sizeof(t)) ==
      -1) {
    debug_printf("Error setting SO_RCVTIMEO option\n");
    return 0;
  }
  if (setsockopt(txSocket, SOL_SOCKET, SO_SNDTIMEO, (char *)&t, sizeof(t)) ==
      -1) {
  ...
  while (*addr) {
    a = *addr;
    debug_printf("Sending request to %d.%d.%d.%d... ", (a & 0xFF),
    ...
    if (sendto(txSocket, SendBuf, BufLen, 0, (SOCKADDR *)&remoteEP,
               sizeof(remoteEP)) == -1) {
      debug_printf("Failed\n");
#ifdef WIN32
...
// Receive response from devices
    ...
    if (status > 0) {
      if (status == 14) {
        ...
        if (serialNumberToFind == device->SerialNumber)
          break;
      } else if (status == 19) {
        // Parse the response
        ...
        device = &devices[nrOfDetectedBoards];
        // device->userID = rcvbuf[0];
        device->SerialNumber = (int)rcvbuf[14] + ((int)rcvbuf[15] << 8) +
                               ((int)rcvbuf[16] << 16) +
                               ((int)rcvbuf[17] << 24);
        device->FirmwareVersionMajor = rcvbuf[3];
        device->FirmwareVersionMinor = rcvbuf[4];
...
sPoKeysDevice *PK_ConnectToNetworkDevice(sPoKeysNetworkDeviceSummary *device) {
  ...
  addr = (uint32)device->IPaddress[0] + ((uint32)device->IPaddress[1] << 8) +
         ((uint32)device->IPaddress[2] << 16) +
         ((uint32)device->IPaddress[3] << 24);
  // Set up the RecvAddr structure with the broadcast ip address and correct
  // port number
...
#ifdef WIN32
  // Set socket options
  setsockopt((SOCKET)tmpDevice->devHandle, SOL_SOCKET, SO_RCVTIMEO, (char *)&t,
             sizeof(t));
  setsockopt((SOCKET)tmpDevice->devHandle, SOL_SOCKET, SO_SNDTIMEO, (char *)&t,
             sizeof(t));
#endif
...
// Connect to target IP
  ...
  if (tmpDevice->connectionParam != PK_ConnectionParam_UDP)
    result =
        connect((SOCKET)tmpDevice->devHandle, (SOCKADDR *)tmpDevice->devHandle2,
  ...
  result = 0;
  if (tmpDevice->connectionParam != PK_ConnectionParam_UDP)
    result = connect(*(int *)tmpDevice->devHandle,
                     (struct sockaddr *)tmpDevice->devHandle2,
                     sizeof(struct sockaddr_in));
  else
    result = 0;
...
int32_t SendEthRequest(sPoKeysDevice *device) {
  ...
  while (1) {
...
#ifdef WIN32
      if (sendto((SOCKET)device->devHandle, (char *)device->request, 64, 0,
                 (SOCKADDR *)device->devHandle2,
                 sizeof(struct sockaddr_in)) == -1)
#else
      if (sendto(*(int *)device->devHandle, (char *)device->request, 64, 0,
                 (SOCKADDR *)device->devHandle2,
                 sizeof(struct sockaddr_in)) == -1)
#endif
      {
    ...
    while (1) {
      ...
      result =
          select((SOCKET)device->devHandle + 1, &fds, NULL, NULL, &stimeout);

      if (result == 0 || result == -1) {
        // Timeout...
        debug_printf("Timeout!");
        if (++retries1 > device->readRetries)
          break;
        continue;
      }
...
// printf("Time: %u\n", (uint32_t)device->reserved64);
      ...
      result =
          select(*(int *)device->devHandle + 1, &fds, NULL, NULL, &stimeout);
    ...
    if (retries2++ > device->sendRetries)
      break;
  }
...
int32_t SendEthRequestBig(sPoKeysDevice *device) {
  ...
  while (1) {
    ...
    if (device->connectionParam == PK_ConnectionParam_UDP) {
#ifdef WIN32
      if (sendto((SOCKET)device->devHandle, (char *)requestBuffer, 512, 0,
                 (SOCKADDR *)device->devHandle2,
                 sizeof(struct sockaddr_in)) == -1)
#else
      if (sendto(*(int *)device->devHandle, (char *)requestBuffer, 512, 0,
                 (SOCKADDR *)device->devHandle2,
                 sizeof(struct sockaddr_in)) == -1)
#endif
      {
...
void PK_SetEthernetRetryCountAndTimeout(sPoKeysDevice *device,
