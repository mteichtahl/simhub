...
void InitializeNewDevice(sPoKeysDevice *device) {
  ...
  device->Pins =
      (sPoKeysPinData *)malloc(sizeof(sPoKeysPinData) * device->info.iPinCount);
  ...
  device->Encoders = (sPoKeysEncoder *)malloc(sizeof(sPoKeysEncoder) *
                                              device->info.iEncodersCount);
  memset(device->Encoders, 0,
         sizeof(sPoKeysEncoder) * device->info.iEncodersCount);

  if (device->info.iEasySensors) {
    device->EasySensors = (sPoKeysEasySensor *)malloc(
        sizeof(sPoKeysEasySensor) * device->info.iEasySensors);
    memset(device->EasySensors, 0,
           sizeof(sPoKeysEasySensor) * device->info.iEasySensors);
  } else {
  ...
  device->PWM.PWMduty =
      (uint32_t *)malloc(sizeof(uint32_t) * device->info.iPWMCount);
  memset(device->PWM.PWMduty, 0, sizeof(uint32_t) * device->info.iPWMCount);

  device->PWM.PWMenabledChannels =
      (unsigned char *)malloc(sizeof(unsigned char) * device->info.iPWMCount);
  memset(device->PWM.PWMenabledChannels, 0,
         sizeof(unsigned char) * device->info.iPWMCount);
  device->PWM.PWMpinIDs =
      (unsigned char *)malloc(sizeof(unsigned char) * device->info.iPWMCount);
  memset(device->PWM.PWMpinIDs, 0,
         sizeof(unsigned char) * device->info.iPWMCount);

  PK_FillPWMPinNumbers(device);

  device->PoExtBusData =
      (unsigned char *)malloc(sizeof(unsigned char) * device->info.iPoExtBus);

  device->MatrixLED = (sPoKeysMatrixLED *)malloc(sizeof(sPoKeysMatrixLED) *
                                                 device->info.iMatrixLED);
  memset(device->MatrixLED, 0,
         sizeof(sPoKeysMatrixLED) * device->info.iMatrixLED);

  memset(&device->PEv2, 0, sizeof(sPoKeysPEv2));

  device->multiPartBuffer = malloc(512);
  if (device->multiPartBuffer <= 0)
    device->multiPartBuffer = 0;
...
void PK_ReleaseDeviceStructure(sPoKeysDevice *device) { CleanDevice(device); }

void PK_CloneDeviceStructure(sPoKeysDevice *original,
                             sPoKeysDevice *destination) {
  // Reserve memory...
  destination->Pins = (sPoKeysPinData *)malloc(sizeof(sPoKeysPinData) *
                                               original->info.iPinCount);
  destination->Encoders = (sPoKeysEncoder *)malloc(
      sizeof(sPoKeysEncoder) * original->info.iEncodersCount);
  destination->PWM.PWMduty =
      (uint32_t *)malloc(sizeof(uint32_t) * original->info.iPWMCount);
  destination->PWM.PWMenabledChannels =
      (unsigned char *)malloc(sizeof(unsigned char) * original->info.iPWMCount);
  destination->PWM.PWMpinIDs =
      (unsigned char *)malloc(sizeof(unsigned char) * original->info.iPWMCount);
  destination->MatrixLED = (sPoKeysMatrixLED *)malloc(
      sizeof(sPoKeysMatrixLED) * original->info.iMatrixLED);

  if (original->info.iEasySensors) {
    destination->EasySensors = (sPoKeysEasySensor *)malloc(
        sizeof(sPoKeysEasySensor) * original->info.iEasySensors);
  } else {
  ...
  if (original->netDeviceData != 0) {
    destination->netDeviceData =
        (sPoKeysNetworkDeviceInfo *)malloc(sizeof(sPoKeysNetworkDeviceInfo));
    memcpy(destination->netDeviceData, original->netDeviceData,
           sizeof(sPoKeysNetworkDeviceInfo));
  } else {
    destination->netDeviceData = 0;
  }
  destination->PoExtBusData =
      (unsigned char *)malloc(sizeof(unsigned char) * original->info.iPoExtBus);
  ...
  destination->UltraFastEncoderConfiguration =
      original->UltraFastEncoderConfiguration;
  ...
  memcpy(destination->PoExtBusData, original->PoExtBusData,
         sizeof(unsigned char) * original->info.iPoExtBus);
...
// bit 0: use UDP
sPoKeysDevice *
PK_ConnectToPoKeysDevice(uint32_t serialNumber,
    ...
    if ((cur_dev->interface_number == 1 && devRange == 0) ||
      ...
      if (cur_dev->serial_number != 0 && cur_dev->serial_number[0] != 'P') {
        ...
        for (k = 1; k < 8 && cur_dev->serial_number[k] != 0; k++) {
          if (cur_dev->serial_number[k] != serialSearch[k])
            break;
        }

        if (deviceTypeRequested == 2 && cur_dev->serial_number[0] != '2')
          k = 0;
        if (deviceTypeRequested == 3 && cur_dev->serial_number[0] != '3')
          k = 0;
        if (deviceTypeRequested == 4 && cur_dev->serial_number[0] != '4')
          k = 0;
        ...
        // Old, PoKeys55 device - we must to connect and read the serial
        // number...
        tmpDevice = (sPoKeysDevice *)malloc(sizeof(sPoKeysDevice));
        tmpDevice->devHandle = (void *)hid_open_path(cur_dev->path);
        tmpDevice->devHandle2 = 0;
  ...
  if (checkForNetworkDevicesAndTimeout) {
    devices = (sPoKeysNetworkDeviceSummary *)malloc(
        sizeof(sPoKeysNetworkDeviceSummary) * 16);
    iNet = PK_SearchNetworkDevices(devices, checkForNetworkDevicesAndTimeout,
                                   serialNumber);
    ...
    for (k = 0; k < iNet; k++) {
      // printf("\nNetwork device found, serial = %lu at %u.%u.%u.%u",
      // devices[k].SerialNumber, devices[k].IPaddress[0],
      // devices[k].IPaddress[1], devices[k].IPaddress[2],
      // devices[k].IPaddress[3]);
...
PK_ConnectToDeviceWSerial(uint32_t serialNumber,
                          uint32_t checkForNetworkDevicesAndTimeout) {
  return PK_ConnectToPoKeysDevice(serialNumber,
                                  checkForNetworkDevicesAndTimeout, 0);
}
...
PK_ConnectToDeviceWSerial_UDP(uint32_t serialNumber,
                              uint32_t checkForNetworkDevicesAndTimeout) {
  return PK_ConnectToPoKeysDevice(serialNumber,
                                  checkForNetworkDevicesAndTimeout, 1);
}
...
int32_t CreateRequest(unsigned char *request, unsigned char type,
...
int32_t PK_CustomRequest(sPoKeysDevice *device, unsigned char type,
                         unsigned char param1, unsigned char param2,
                         unsigned char param3, unsigned char param4) {
...
int32_t SendRequest(sPoKeysDevice *device) {
  ...
  while (retries++ < 2) {
    ...
    while (waits++ < 50) {
      ...
      if (device->response[0] == 0xAA &&
          device->response[6] == device->requestID) {
...
// ******************************************************************************************
// These functions are used to access the device data without using the
// structures...
// ******************************************************************************************
